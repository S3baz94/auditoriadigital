...
// Inlined schema so it's always available inside Vercel's serverless bundle
const SCHEMA_SQL = `
...
export function getDbPath(): string {
  // Vercel serverless: filesystem is ephemeral — use /tmp so SQLite can write.
  if (process.env.VERCEL) return "/tmp/leadpulse.db";
  const dataDir = path.join(process.cwd(), "data");
  if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
  return path.join(dataDir, "leadpulse.db");
}
